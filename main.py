...
import webapp2, re, urllib2, PyRSS2Gen, datetime, cgi
...
class WodEntry(db.Model):
    ...
    def desc(self):
        ret = "<h2>Workout</h2>%s<h2>Result</h2>%s<br/><br/>%s" % (self.workout_desc, self.result, self.workout_details.replace("NA</span>", "</span>"))
        if self.notes:
...
class Main(webapp2.RequestHandler):
    ...
    footer =      """<p><i>This site is not affiliated with or endorsed by the beyondthewhiteboard.com</i></p></body>
                </html>"""
    
    def get(self):
        html = self.header + """
                  <p>Enter your beyondthewhiteboard id:</p>
                  <p><img src="http://farm8.staticflickr.com/7035/6669670393_a789421208_o.png" alt="finding your beyondthewhiteboard id" border="1"></p>
...
class Wods(webapp2.RequestHandler):
    ...
    def workout_details(self, url):
        ...
        return workout.find('div', { 'class' : 'workout_description_container' })
    ...
    def workout_post(self, url, id):
        ...
        if query.count() > 0:
            ...
            entry.workout_name = "%s".decode("utf-8") % workout_url.string.strip()
            ...
            entry.date = datetime.datetime.strptime(completed_date, '%A, %B %d, %Y')
    ...
    def workout_posts(self, id):
        soup = self.soup_url("%s/members/%s/workout_sessions" % (self.site,id))
        user = soup.find('div', { 'class' : 'name group' }).find('a').string.strip()
        entries = []
        groups = soup.findAll('dl', { 'class' : 'workout_session activity group'})
        for group in groups:
            wod_url = "http://beyondthewhiteboard.com%s" % group.find('span', { 'class' : 'title_session_result'}).find('a')['href']
